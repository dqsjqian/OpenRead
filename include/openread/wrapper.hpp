#pragma once
/// @file wrapper.hpp
/// @brief OpenRead C++ 便捷绑定 —— 供 Qt / 其他 C++ GUI 直接使用
///
...
class EngineWrapper {
public:
    EngineWrapper() : handle_(openread_engine_create()) {
        if (!handle_) {
            throw std::runtime_error("Failed to create OpenRead engine");
        }
    }

    ~EngineWrapper() {
        if (handle_) {
            openread_engine_destroy(handle_);
        }
    }
    ...
    EngineWrapper& operator=(EngineWrapper&& other) noexcept {
        if (this != &other) {
            if (handle_) openread_engine_destroy(handle_);
    ...
    void setHttpCallback(HttpCallback callback) {
        ...
        if (httpCallback_) {
            openread_engine_set_http_callback(
                handle_,
            ...
            openread_engine_set_http_callback(handle_, nullptr, nullptr);
        }
    }
    ...
    void setLogCallback(LogCallback callback) {
        ...
        if (logCallback_) {
            openread_engine_set_log_callback(
                handle_,
            ...
            openread_engine_set_log_callback(handle_, nullptr, nullptr);
        }
    }
    ...
    bool loadSource(const std::string& json) {
        return openread_engine_load_source(handle_, json.c_str()) == 0;
    }

    int loadSources(const std::string& jsonArray) {
        return openread_engine_load_sources(handle_, jsonArray.c_str());
    }

    int loadSourcesFromFile(const std::string& filePath) {
        return openread_engine_load_sources_from_file(handle_, filePath.c_str());
    }

    bool selectSource(int index) {
        return openread_engine_select_source(handle_, index) == 0;
    }

    bool selectSourceByName(const std::string& name) {
        return openread_engine_select_source_by_name(handle_, name.c_str()) == 0;
    }

    int sourceCount() const {
        return openread_engine_source_count(handle_);
    }
    ...
    std::vector<Book> search(const std::string& keyword) {
        char* result = openread_engine_search(handle_, keyword.c_str());
    ...
    std::vector<Chapter> getCatalog(const std::string& bookUrl) {
        char* result = openread_engine_get_catalog(handle_, bookUrl.c_str());
    ...
    std::string getContent(const std::string& chapterUrl) {
        char* result = openread_engine_get_content(handle_, chapterUrl.c_str());
        std::string content;
        if (result) {
            content = result;
            openread_free_string(result);
    ...
    std::vector<Chapter> getCatalogForSource(const std::string& bookUrl, int sourceIndex, const std::string& sourceName = "") {
        char* result = openread_engine_get_catalog_for_source(handle_, bookUrl.c_str(), sourceIndex, sourceName.c_str());
        std::vector<Chapter> chapters;
        if (result) {
            chapters = parseChapters(result);
            openread_free_string(result);
    ...
    std::string getContentForSource(const std::string& chapterUrl, int sourceIndex, const std::string& sourceName = "") {
        char* result = openread_engine_get_content_for_source(handle_, chapterUrl.c_str(), sourceIndex, sourceName.c_str());
        std::string content;
        if (result) {
            content = result;
            openread_free_string(result);
    ...
    void setDatabasePath(const std::string& dbPath) {
        openread_engine_set_database_path(handle_, dbPath.c_str());
    }

    int loadSourcesFromDatabase() {
        return openread_engine_load_sources_from_database(handle_);
    }

    int removeInvalidSources() {
        return openread_engine_remove_invalid_sources(handle_);
    }

    int clearAllSources() {
        return openread_engine_clear_all_sources(handle_);
    }
    ...
    int64_t addToBookshelf(const std::string& bookJson) {
        return openread_bookshelf_add(handle_, bookJson.c_str());
    }

    bool removeFromBookshelf(const std::string& bookUrl, const std::string& sourceUrl = "") {
        return openread_bookshelf_remove(handle_, bookUrl.c_str(), sourceUrl.c_str()) == 0;
    }

    std::string getBookshelfJson() {
        char* result = openread_bookshelf_list(handle_);
        std::string json;
        if (result) {
            json = result;
            openread_free_string(result);
    ...
    bool isInBookshelf(const std::string& bookUrl, const std::string& sourceUrl = "") {
        return openread_bookshelf_is_in(handle_, bookUrl.c_str(), sourceUrl.c_str()) != 0;
    }

    bool saveReadProgress(const std::string& progressJson) {
        return openread_bookshelf_progress_save(handle_, progressJson.c_str()) == 0;
    }

    std::string getReadProgressJson(const std::string& bookUrl, const std::string& sourceUrl = "") {
        char* result = openread_bookshelf_progress_get(handle_, bookUrl.c_str(), sourceUrl.c_str());
        std::string json;
        if (result) {
            json = result;
            openread_free_string(result);
    ...
    bool changeBookSource(const std::string& bookUrl, const std::string& oldSourceUrl,
        ...
        return openread_bookshelf_change_source(handle_,
    ...
    int checkBookUpdate(const std::string& bookUrl, const std::string& sourceUrl,
        ...
        return openread_bookshelf_check_update(handle_,
    ...
    int updateBookshelfLastChapter(const std::string& bookUrl, const std::string& sourceUrl,
        ...
        return openread_bookshelf_update_last_chapter(handle_,
    ...
    std::string downloadBook(const std::string& bookUrl, const std::string& sourceUrl,
        ...
        char* result = openread_bookshelf_download(handle_,
            bookUrl.c_str(), sourceUrl.c_str(), sourceIndex, sourceName.c_str(), sleepMs);
        std::string json;
        if (result) { json = result; openread_free_string(result); }
        return json;
    }

    int checkAllUpdates() {
        return openread_bookshelf_check_all_updates(handle_);
    }

    void setConcurrency(int n) {
        openread_engine_set_concurrency(handle_, n);
    }
    ...
    void validateConcurrent(
        ...
        openread_engine_validate_concurrent(
    ...
    void searchAllConcurrent(
        ...
        openread_engine_search_all_concurrent(
    ...
    std::string getSourceInfo() {
        char* result = openread_engine_get_source_info(handle_);
        std::string info;
        if (result) {
            info = result;
            openread_free_string(result);
    ...
    std::string getLastError() const {
        return openread_engine_get_last_error(handle_);
    }

    bool isAvailable() const {
        return openread_engine_is_available(handle_) != 0;
    }
    ...
    OpenReadEngine nativeHandle() const {
        return handle_;
    }

private:
    OpenReadEngine handle_;
