...
/// @brief OpenRead 搜索 ViewModel —— 基于 Aria 响应式 MVVM 框架。
///
/// 流式版本：还原 OpenRead "多源并发搜索、结果慢慢显示出来" 的体验。
...
using SearchEmit = std::function<void(const std::vector<openread::Book>& books)>;
...
public:
    ...
    aria::ObservableList<openread::Book> results;
...
private:
    ...
    void append_results_(const std::vector<openread::Book>& books);
