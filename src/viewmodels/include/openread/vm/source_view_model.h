...
/// @brief OpenRead 书源管理 ViewModel —— 列表 / 加载 / 验证 / 清理。
///
...
public:
    ...
    virtual openread::SourceListResult get_source_list() = 0;
    ...
    virtual void validate_sources(
        ...
        std::shared_ptr<openread::CancelToken> cancelToken) = 0;
...
public:
    ...
    // ── 响应式状态（UI 绑定）──────────────────────────────────────
    /// 书源列表
    aria::ObservableList<openread::SourceSummary> sources;
...
private:
    void apply_list_(const openread::SourceListResult& result);
    ...
    // 验证取消令牌
    std::shared_ptr<openread::CancelToken> cancel_token_;
};
