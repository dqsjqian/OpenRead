...
/// @brief OpenRead 书架 ViewModel —— 基于 Aria 响应式 MVVM 框架。
///
...
public:
    ...
    /// 加载书架全部条目（含缓存状态与阅读进度）。
    virtual std::vector<openread::BookshelfDetail> load() = 0;

    /// 移除一本书（按 bookUrl）。返回是否成功。
    virtual bool remove(const std::string& bookUrl) = 0;

    /// 加入一本书。返回新插入 id（已存在返回 -1）。
    virtual int64_t add(const openread::BookshelfItem& item) = 0;
};
...
public:
    ...
    aria::ObservableList<openread::BookshelfDetail> books;
    ...
    /// 加入一本书并自动刷新。
    void add(const openread::BookshelfItem& item);

private:
    void reload_into_list_(const std::vector<openread::BookshelfDetail>& items);
    ...
    aria::async::AsyncCommand<int64_t, openread::BookshelfItem> add_command_;
};
