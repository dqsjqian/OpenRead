...
/// @brief OpenRead 阅读器 ViewModel —— 目录 / 正文 / 阅读进度。
///
...
public:
    ...
    virtual std::vector<openread::Chapter> load_catalog(
    ...
    /// 保存阅读进度。
    virtual void save_progress(const openread::ReadProgress& progress) = 0;

    /// 读取阅读进度。
    virtual openread::ReadProgress load_progress(const std::string& bookUrl) = 0;
};
...
public:
    ...
    aria::ObservableList<openread::Chapter> chapters;
    /// 章节总数
...
private:
    void apply_catalog_(const std::vector<openread::Chapter>& chs);
