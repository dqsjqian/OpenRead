...
```bash
python3 tools/ci/build_openread_deps.py     # 取固定版本依赖（只写 build/deps）
...
```bash
python3 tools/ci/build_openread_deps.py            # 首次构建（约 10 分钟，主要是 OpenSSL）
cmake -S . -B build -DOPENREAD_BUILD_TESTS=ON      # 自动探测 build/deps/prefix
cmake --build build
ctest --test-dir build --output-on-failure
```

自定义前缀用 `-DOPENREAD_DEPS_PREFIX=<prefix>`。脚本 `--offline` 只走缓存、
